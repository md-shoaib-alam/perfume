...
export const AccountDashboard: React.FC<AccountDashboardProps> = ({
  ...
  // Wishlist & Orders States from local storage
  ...
  const handleSendPasswordReset = async () => {
    try {
      ...
      setTimeout(() => {
        setPwdMsg(`Password reset link has been dispatched to ${email}. Please check your inbox.`);
      }, 800);
    } catch (e) {
      setPwdMsg('Failed to trigger password reset.');
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/70 backdrop-blur-xs overflow-y-auto animate-fade-in-up">
      <div className="relative w-full max-w-6xl bg-white rounded-md shadow-2xl border border-slate-100 overflow-hidden my-auto min-h-[620px] flex flex-col font-sans">
        ...
        {/* Dashboard Main Grid: Left Sidebar + Right Content */}
        <div className="flex-1 flex flex-col md:flex-row min-h-0">
          ...
          <main className="flex-1 p-6 sm:p-9 overflow-y-auto bg-slate-50/50">
            ...
            {/* ----------------------------------------------------------- */}
            {/* TAB 1: DASHBOARD OVERVIEW (Exact Screenshot Match)          */}
            {/* ----------------------------------------------------------- */}
            {activeTab === 'dashboard' && (
              <div className="space-y-6">
                <div>
                  ...
                  {/* 4 Top Metric Summary Cards with Professional SVG Icons */}
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                    ...
                    <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-100 shadow-2xs flex items-center justify-between">
                      <div>
                        ...
                        <h4 className="text-xl sm:text-2xl font-bold text-slate-900">Rs.0.00</h4>
                      </div>
                ...
                {/* "Your Profile" Card (Screenshot-Matched) */}
                      ...
                      <div className="flex items-center gap-4">
                        <div className="w-16 h-16 rounded-full border-2 border-[#d09e44] p-1 flex items-center justify-center bg-white shadow-2xs">
                          {user?.imageUrl ? (
                            ...
                            <span className="text-xl font-serif text-slate-500">{firstName ? firstName[0] : '?'}</span>
                          )}
                        </div>
                        <div>
                ...
                <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-2xs">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={() => setActiveTab('orders')} className="text-xs text-[#d09e44] font-semibold hover:underline cursor-pointer">
                      View All
                    </button>
                  </div>
                  {userOrders.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {userOrders.slice(0, 3).map((o, idx) => (
                        <div key={idx} className="p-3 bg-slate-50 rounded-xl flex items-center justify-between text-xs">
                          <span className="font-semibold">Order #{o.id || idx + 1001}</span>
                          <span className="text-slate-500">Rs.{o.total}</span>
                          <span className="text-emerald-600 font-bold">{o.status || 'Confirmed'}</span>
                        </div>
                      ))}
                    </div>
                  )}
              ...
              <div className="space-y-4">
                ...
                {userOrders.length === 0 ? (
                  <div className="bg-white rounded-2xl p-10 border border-slate-100 text-center text-slate-400">
                    ...
                    <button
                      onClick={onClose}
                      className="px-5 py-2.5 bg-[#d8a753] hover:bg-[#c69542] text-white font-bold text-xs rounded-xl shadow-xs cursor-pointer"
                    >
                      Shop Now
                    </button>
                  </div>
                  ...
                  <div className="space-y-3">
                    {userOrders.map((ord, i) => (
                      <div key={i} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-2xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                          <p className="text-xs font-bold text-slate-900">Order #{ord.id || `NSH-00${i + 1}`}</p>
                          <p className="text-[11px] text-slate-400">{new Date().toLocaleDateString('en-IN', { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                        </div>
                        <div className="text-xs font-bold text-slate-800">
                          Rs.{ord.total || '2,450.00'}
                        </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {wishlist.map((prod) => (
                      <div key={prod.id} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-2xs flex flex-col justify-between">
                        <img src={prod.image} alt={prod.name} className="w-full h-36 object-cover rounded-xl mb-3" />
                        <div>
            ...
            {activeTab === 'memberships' && (
              <div className="bg-white rounded-2xl p-6 sm:p-8 border border-slate-100 shadow-2xs space-y-6">
                ...
                <div className="p-5 bg-gradient-to-r from-slate-900 to-slate-800 text-white rounded-2xl flex flex-col justify-between h-44 relative overflow-hidden shadow-md">
                  <div className="flex justify-between items-start">
                    <div>
                      <span className="font-serif text-lg font-bold tracking-widest text-[#d6a750]">NEESH VIP</span>
                      <p className="text-[11px] text-slate-400 mt-0.5">Tier: Silver Member</p>
                    </div>
                    <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-[#d6a750]">
                      <svg className="w-4 h-4 fill-none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z" />
                      </svg>
                    </div>
                  ...
                  <div>
                    <p className="text-xs font-medium text-slate-300 mb-1.5">{displayName}</p>
                    <div className="w-full bg-white/20 h-1.5 rounded-full overflow-hidden">
                      <div className="bg-[#d6a750] h-full w-[25%]" />
                    </div>
                    <p className="text-[10px] text-slate-400 mt-1">450 points to Gold VIP Tier</p>
                  </div>
                </div>
              </div>
            )}
