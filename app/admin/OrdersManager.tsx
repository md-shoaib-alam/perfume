...
const STATUS_OPTIONS = [
  { value: 'Order Placed', label: '1. Order Placed' },
  { value: 'Packed', label: '2. Packed' },
  { value: 'Shipped', label: '3. Shipped' },
  { value: 'Out for Delivery', label: '4. Out for Delivery' },
...
const OrderStatusPill: React.FC<{
...
}> = ({ currentStatus, onSelect, getStatusBadge }) => {
  ...
  if (isCancelled) {
    return (
      <div
        className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-rose-50 text-rose-800 border border-rose-200 text-xs font-semibold select-none cursor-not-allowed opacity-90 shadow-2xs"
        title="Order cancelled & permanently locked"
      >
  ...
  const activeOption = STATUS_OPTIONS.find(
    (opt) => opt.value.toLowerCase() === (currentStatus || '').toLowerCase()
  ...
  return (
    <div className="relative inline-block text-left" ref={dropdownRef}>
      <button
        ...
        className={`inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1 rounded-full border transition-all shadow-2xs cursor-pointer select-none ${getStatusBadge(
          currentStatus
        )}`}
      >
        <span>{activeOption.label}</span>
        <svg
          className={`w-3 h-3 text-current opacity-70 transition-transform duration-150 shrink-0 ${
            isOpen ? 'rotate-180' : ''
          }`}
          fill="none"
          viewBox="0 0 24 24"
      ...
      {isOpen && (
        <div className="absolute left-0 top-full mt-1.5 w-44 bg-white rounded-xl shadow-xl border border-slate-200 py-1.5 z-50 animate-in fade-in zoom-in-95 duration-100">
          {STATUS_OPTIONS.map((opt) => {
            const isSelected = opt.value.toLowerCase() === (currentStatus || '').toLowerCase();
            const isCancelOpt = opt.value === 'Cancelled';
            return (
              <button
                key={opt.value}
                type="button"
...
export const OrdersManager: React.FC = () => {
  ...
  const handleStatusChange = async (orderId: string, newStatus: string) => {
    // 1. Warning confirmation before marking order as Cancelled
    if (newStatus.toLowerCase() === 'cancelled') {
      const confirmed = await showConfirm({
        title: 'Cancel Customer Order?',
        message: 'Are you sure you want to cancel this order? Once cancelled, the order is permanently locked and cannot be processed further.',
        confirmText: 'Yes, Cancel Order',
        variant: 'danger'
      });
      if (!confirmed) {
        // Revert local state render by refreshing orders without full reload
        setOrders((prev) => [...prev]);
        return;
      }
    }
  ...
  const handleSaveOrderTracking = async (e: React.FormEvent) => {
    ...
    // Warning confirmation before marking order as Cancelled in modal
    const prevStatus = (selectedOrder.orderStatus || selectedOrder.status || '').toLowerCase();
    if (modalStatus.toLowerCase() === 'cancelled' && prevStatus !== 'cancelled') {
      const confirmed = await showConfirm({
        title: 'Cancel Customer Order?',
  ...
  const getStatusBadge = (status: string) => {
    ...
    if (s.includes('shipped') || s.includes('transit') || s.includes('out for delivery')) {
      return 'bg-blue-50 text-blue-800 border-blue-200 hover:border-blue-300';
    }
    if (s.includes('order placed') || s.includes('packed') || s.includes('processing') || s.includes('pending')) {
      return 'bg-amber-50 text-amber-900 border-amber-200 hover:border-amber-300';
    }
  ...
  return (
    <div className="space-y-5 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
        <div>
          <h2 className="text-xl font-bold text-slate-900 tracking-tight">Customer Orders</h2>
              ...
              >
                ...
                <div className="text-xs py-0.5">
                  ...
                  <p className="text-[10.5px] text-slate-500 font-mono mt-0.5">
                    {cust.phone || ord.customerPhone || ''} {cust.email ? `• ${cust.email}` : ''}
                  </p>
                </div>
                ...
                <div className="flex items-center justify-between gap-2 pt-2 border-t border-slate-100">
                  <OrderStatusPill
                    currentStatus={ord.orderStatus || ord.status || 'Order Placed'}
                    onSelect={(newStatus) => handleStatusChange(ord._id || ord.orderNumber, newStatus)}
                ...
                paginatedOrders.map((ord) => {
                  ...
                  return (
                    <tr key={ord._id || ord.orderNumber} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      {/* Clean Customer Info Column */}
                      <td className="px-5 py-3.5 align-middle">
                        <div className="font-bold text-slate-900 text-xs">
                          {cust.name || ord.customerName || 'Customer'}
                        </div>
                        <div className="text-[10.5px] font-mono text-slate-500 mt-0.5 flex items-center gap-1.5 flex-wrap">
                          <span>{cust.phone || ord.customerPhone || 'No phone'}</span>
                          {(cust.email || ord.customerEmail) && (
                            <>
                              <span className="text-slate-300">•</span>
                              <span className="truncate max-w-[200px]">{cust.email || ord.customerEmail}</span>
                            </>
                          )}
                        </div>
                      </td>
                      ...
                      {((selectedOrder.orderStatus || selectedOrder.status || '').toLowerCase() === 'cancelled') ? (
                        ...
                        <div className="relative">
                          <select
                            ...
                            className="w-full appearance-none bg-white border border-slate-200 focus:border-[#d6a750] rounded-xl px-3.5 pr-8 py-2 text-xs font-bold text-slate-800 focus:outline-none transition-all cursor-pointer"
                          >
