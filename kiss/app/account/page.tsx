...
export default function AccountPage() {
  ...
  const handleAddToCart = (product: Product, size: string) => {
    setCartItems((prev) => {
      ...
      if (existing) {
        return prev.map((item) => item.product.id === product.id && item.selectedSize === size ? { ...item, quantity: item.quantity + 1 } : item);
      }
    ...
    <div className="min-h-screen bg-[#fafafa] text-slate-900 font-sans flex flex-col justify-between selection:bg-[#d6a13d] selection:text-black">
        ...
        <div className="bg-white rounded-md shadow-md border border-slate-100 overflow-hidden flex flex-col md:flex-row min-h-[640px]">
          ...
          <aside className="w-full md:w-64 bg-white border-r border-slate-100 p-6 flex flex-col shrink-0">
            ...
            <div className="flex flex-col items-center text-center pb-6 border-b border-slate-100">
              <div className="relative w-22 h-22 rounded-full border-2 border-[#d09e44] p-1 flex items-center justify-center bg-white shadow-2xs mb-3">
                {user?.imageUrl ? (
                  <img
                    src={user.imageUrl}
                    alt={displayName}
            ...
            {/* Navigation Tab Links with SVG Icons */}
              ...
              <div className="space-y-4">
                ...
                {userOrders.length === 0 ? (
                  <div className="bg-white rounded-2xl p-10 border border-slate-100 text-center text-slate-400">
                    ...
                    <Link
                      href="/"
                      className="inline-block px-5 py-2.5 bg-[#d8a753] hover:bg-[#c69542] text-white font-bold text-xs rounded-xl shadow-xs cursor-pointer"
                    >
                      Shop Now
                    </Link>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {wishlist.map((prod) => (
                      <div key={prod.id} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-2xs flex flex-col justify-between">
                        ...
                        <button
                          onClick={() => handleAddToCart(prod, '100ml')}
      ...
      <CartDrawer
        ...
        onUpdateQuantity={(id, delta) => {
          setCartItems(prev => prev.map(item => item.product.id === id ? { ...item, quantity: item.quantity + delta } : item).filter(Boolean) as CartItem[]);
        }}
        onRemoveItem={(id) => {
