              ...
              >
                ...
                <div className="flex items-center justify-between gap-2 pt-2 border-t border-slate-100">
                  <div className="relative inline-flex items-center">
                    <select
                      value={ord.orderStatus || ord.status || 'Order Placed'}
                      onChange={(e) => handleStatusChange(ord._id || ord.orderNumber, e.target.value)}
                      className={`appearance-none text-[11.5px] font-semibold pl-3 pr-7 py-1 rounded-full border transition-all shadow-2xs focus:outline-none focus:ring-1 cursor-pointer select-none ${getStatusBadge(
                    ...
                    <div className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 flex items-center">
                      <svg className="w-3 h-3 text-current opacity-70" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                      ...
                      <td className="px-4 py-3.5 align-middle">
                        <div className="relative inline-flex items-center">
                          <select
                            value={ord.orderStatus || ord.status || 'Order Placed'}
                            onChange={(e) => handleStatusChange(ord._id || ord.orderNumber, e.target.value)}
                            className={`appearance-none text-xs font-semibold pl-3.5 pr-8 py-1.5 rounded-full border transition-all shadow-2xs focus:outline-none focus:ring-1 cursor-pointer select-none ${getStatusBadge(
                              ord.orderStatus || ord.status || 'Order Placed'
                            )}`}
                          >
                            <option value="Order Placed">1. Order Placed</option>
